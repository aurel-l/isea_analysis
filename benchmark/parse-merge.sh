...
do
  ...
  then
    ...
    mem=$(grep "Maximum resident set size" statsMerge/$fileName)
    [[ $mem =~ $regexMem ]]
    megs=$(bc -l <<< "${BASH_REMATCH[1]}/1000")
    printf "${megs}, "
    
    exit=$(grep "Exit status" statsMerge/$fileName)
    [[ $exit =~ $regexExit ]]
