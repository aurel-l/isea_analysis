...
demesToIslands = compiler::cmpfun(function(df, summaryNames, islands) {
    ...
    for (i in islands) {
        s = df[df$Island == i, ]
        output[output$Island == i, summaryNames] = colMeans(
...
#
#     if (nrow(current) == 0L) {
#         compared$admixture = factor(compared$admixture)
#         return(compared)
#     } else {
#         return(rbind(current, compared))
#     }
# })
